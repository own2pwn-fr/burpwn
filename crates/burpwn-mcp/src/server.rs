...
impl BurpwnServer {
    ...
    #[tool(
        description = "Archive the WHOLE session into one portable file (a `.burpwn` bundle): every captured flow with its bodies, plus the workspaces, groups, tags, notes, attacks and match/replace rules. Reach for it when a piece of work is finished and worth keeping or handing over — the file opens on another machine with `burpwn session import <file>`, which is an operator action, not a tool. WARNING: by default the bundle is RAW, so it carries the stored auth tokens, the login commands and the Authorization/Cookie headers captured in the traffic; pass redact=true to drop the stored credentials AND mask the credential-shaped values in the capture (Authorization/Proxy-Authorization/Cookie/Set-Cookie headers, password/token/api_key-style parameters) — it matches shapes, so a secret under a name it cannot recognise, one baked into a URL path, or one inside a binary body is still in the file. Refuses to overwrite an existing file unless force=true."
    )]
