...
if ! "$BURPWN" doctor; then
  ...
  # No WSL-specific verdict here on purpose: `doctor` runs a REAL throwaway
  # sandbox and already prints per-step remediation (including the WSL "no
  # loadable modules" case when — and only when — a module-backed step is what
  # failed). A blanket "WSL cannot work" warning printed here contradicted a
  # doctor report whose dummy/nft_redirect steps had just passed.
fi
