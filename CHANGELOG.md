...
one — the `exec` hooks that carry the login macro no longer decode, by design.
...
The test now reads the staged database as **bytes** and fails if the password appears anywhere in
it, which is the property the flag actually promises. What clears the residue is the post-redaction
`VACUUM INTO` described above: the file that gets compressed is a second copy rebuilt from the
surviving rows, so the pages the redaction freed are not in it at all. `PRAGMA secure_delete` was
tried first and is *not* enough on its own — it zeroes the cells the `UPDATE`s free, but the traffic
scrub also merges and deletes blob rows, and with the vacuum removed a captured token is still
readable in the bundle. Measured both ways, in `a_redacted_bundle_carries_no_freelist_residue`.

The v8 migration keeps its own `secure_delete` where it drops `auth_profiles`, because that one runs
on the **live** `session.db`, which nothing vacuums afterwards (with the pragma restored after the
migration — that connection is the store's long-lived writer, and the capture path should not pay
for the zero-fill).
